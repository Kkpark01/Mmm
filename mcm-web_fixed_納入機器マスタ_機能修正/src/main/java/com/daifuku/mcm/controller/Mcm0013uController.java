...
public class Mcm0013uController extends BaseController {
    ...
    @GetMapping
    public String index(@RequestParam Long plantId,@RequestParam(required=false) String returnTo0012Url,Model m,HttpSession s) {
    ...
    public String update(@ModelAttribute("input") Mcm0013uForm input,BindingResult b,Model m,HttpSession s,RedirectAttributes ra) {
        synchronized(s) {
            ...
            if(errors.isEmpty()) {
                ...
                try {errors=service.saveWorking(f.copy(),login(s));}
            ...
            String dest=back(f)+(f.getReturnTo0012Url()==null?"":"&returnTo0012Url="+encode(f.getReturnTo0012Url()));clear(s);
