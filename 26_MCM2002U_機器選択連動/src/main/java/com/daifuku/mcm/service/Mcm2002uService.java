...
public class Mcm2002uService {
    ...
    private void applyKotaiCheckLogic(SearchResult result) {
        ...
        // 明細のチェック済みIDセット
        Set<List<BigDecimal>> checkedMeisaiIds = result.getMeisaiRows().stream()
            .filter(r -> isChecked(r.getCheckFlg()))
            .filter(r -> r.getKikikoseiId() != null && r.getKikimeisaiId() != null)
            .map(r -> List.of(r.getKikikoseiId().stripTrailingZeros(), r.getKikimeisaiId().stripTrailingZeros()))
            .collect(Collectors.toSet());

        // 個体行について: 親構成・親明細が両方チェックされている場合のみチェック可能
        for (Mcm2002uKotaiRowDto kotai : result.getKotaiRows()) {
            if (kotai.getMaeKikikoseiId() == null || kotai.getMafKikimeisaiId() == null
                || !checkedKoseiIds.contains(kotai.getMaeKikikoseiId().stripTrailingZeros())
                || !checkedMeisaiIds.contains(List.of(kotai.getMaeKikikoseiId().stripTrailingZeros(), kotai.getMafKikimeisaiId().stripTrailingZeros()))) {
