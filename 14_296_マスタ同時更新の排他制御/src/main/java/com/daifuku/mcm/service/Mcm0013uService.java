...
public class Mcm0013uService {
    ...
    // #296: 排他制御時の共通メッセージ（既存の複数箇所で使用していた文言を集約）
    private static final String MSG_EXCLUSIVE_CONTROL =
        "他のユーザがデータを変更した可能性があります。処理をやり直してください。";

    // #296: 更新日時が一致するか判定する共通ヘルパー
    //
    // 【#296 調査用ログ追加】「排他エラーにならない」事象の切り分け用に、
    //   判定に使った実値を必ず残す。読み方は次のとおり。
    //     current(dbValue)     : DBから読み直した現在値
    //     base(screenValue)    : 画面／セッションが保持していた表示時点の値
    //     same=true かつ両者が同値 → 競合なしと判定される（＝画面保持値が既に最新）
    //     same=true かつ両方 null → LASTUPDATE_DT が未設定で判定が効いていない
    //     base=null のみ         → 画面・DTO往復のどこかで排他キーが欠落している
    private static boolean isSameLastupdateDt(LocalDateTime current, LocalDateTime base) {
        boolean same;
        if (current == null && base == null) same = true;
        else if (current == null || base == null) same = false;
        else same = current.equals(base);
        log.info("MCM0013U 排他判定 dbValue={} screenValue={} same={}", current, base, same);
        return same;
    }
    ...
    public List<String> checkRequired(KoseiRowForm r) {
        ...
        // 機器個体グリッド（validateKotaiFields）と同じ「○○は必ず入力してください。」形式に統一する。
        var e=first(r.getHyojijun()==null?"Noは必ず入力してください。":null,
            isEmpty(r.getKikikoseiNk())?"機器構成名は必ず入力してください。":null,
            r.getSetNm()==null?"セット数は必ず入力してください。":null,
            isEmpty(r.getTani())?"単位は必ず入力してください。":null);
    ...
    // #304: VBの明細設定戻りと同様に、有効明細のフラグから構成側をONにする。
    // 数量チェック・個体管理対象判定とは独立。非コントローラだけの場合は値を変えない。
    private void enableWorkingController(Mcm0013uForm f,KoseiRowForm parent) {
        if(!"1".equals(parent.getControllerFlg())&&activeDetails(f,parent.getKikikoseiId()).stream()
            .anyMatch(d->d.getControllerFlg()!=null&&d.getControllerFlg().intValue()==1)) {
    ...
    public void syncWorkingIndividuals(Mcm0013uForm f,Long id) {
        ...
        // 14U設定の受入れ時にも呼ばれる。個体行生成ができない状態でもフラグを反映する。
        enableWorkingController(f,parent);
        if(parent.getSetNm()==null||parent.getSetNm()<1||parent.getSetNm()>9999) return;
    ...
    public List<String> saveWorking(Mcm0013uForm form,String user) {
        ...
        // #296追加: 削除実行前に、DB上の現在値が表示時点から変わっていないか（＝他ユーザーが
        // 既に更新／削除していないか）を確認する。既存の存在チェック(orElseThrow相当)と統合。
        for(var k:form.getKotaikanriRows()) if(!active(k.getRowStatus())&&k.getKotaikanriId()>0) {
            var existing=kotaiRepo.findById(BigDecimal.valueOf(k.getKotaikanriId()))
                .orElseThrow(()->new IllegalStateException(MSG_EXCLUSIVE_CONTROL));
            if(!isSameLastupdateDt(existing.getLastupdateDt(), k.getLastupdateDt()))
                throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
            claimVersion("MCM_MA_KIKIKOTAIKANRI", "KOTAIKANRI_ID", longToBd(k.getKotaikanriId()), k.getLastupdateDt());
            kotaiRepo.deleteById(longToBd(k.getKotaikanriId()));
        }
        kotaiRepo.flush();
        for(var entries:form.getDetails().values()) for(var d:entries) if(!active(d.getRowStatus())&&d.getKikimeisaiId().signum()>0) {
            var existing=meisaiRepo.findById(d.getKikimeisaiId())
                .orElseThrow(()->new IllegalStateException(MSG_EXCLUSIVE_CONTROL));
            if(!isSameLastupdateDt(existing.getLastupdateDt(), d.getLastupdateDt()))
                throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
            claimVersion("MCM_MA_KIKIMEISAI", "KIKIMEISAI_ID", d.getKikimeisaiId(), d.getLastupdateDt());
            meisaiRepo.deleteById(d.getKikimeisaiId());
        }
        meisaiRepo.flush();
        for(var r:form.getKoseiRows()) if(!active(r.getRowStatus())&&r.getKikikoseiId()>0) {
            BigDecimal id=longToBd(r.getKikikoseiId());
            var existing=koseiRepo.findByPlantIdAndKikikoseiId(longToBd(form.getPlantId()),id);
            if(existing==null) throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
            if(!isSameLastupdateDt(existing.getLastupdateDt(), r.getLastupdateDt()))
                throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
            ...
            claimKoseiVersion(form.getPlantId(), id, r.getLastupdateDt());
        ...
        for(var entry:form.getDetails().entrySet()) {
            ...
            for(var d:entry.getValue()) if(active(d.getRowStatus())&&changed(d.getRowStatus())) {
                ...
                var entity=isNew?new KikimeisaiEntity():meisaiRepo.findById(d.getKikimeisaiId()).orElseThrow(()->new IllegalStateException(MSG_EXCLUSIVE_CONTROL));
                // #296追加: MeisaiRow.lastupdateDt は実装済みのため、そのまま比較可能
                if(!isNew && !isSameLastupdateDt(entity.getLastupdateDt(), d.getLastupdateDt())) {
                    throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
                }
                ...
                if (!isNew) entity.setLastupdateDt(claimVersion("MCM_MA_KIKIMEISAI", "KIKIMEISAI_ID", d.getKikimeisaiId(), d.getLastupdateDt()));
    ...
    /** 同一トランザクション内で比較と更新を一体化し、JPA保存まで行を確保する。 */
    private LocalDateTime claimVersion(String table, String key, BigDecimal id, LocalDateTime expected) {
        LocalDateTime next = com.daifuku.mcm.common.ExclusiveLockKey.nextVersion(expected);
        int count = jdbc.update("UPDATE MCM." + table + " SET LASTUPDATE_DT=? WHERE " + key
                + "=? AND ((? IS NULL AND LASTUPDATE_DT IS NULL) OR LASTUPDATE_DT=?)",
                next, id, expected, expected);
        if (count != 1) throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
        return next;
    }

    private LocalDateTime claimKoseiVersion(Long plantId, BigDecimal id, LocalDateTime expected) {
        LocalDateTime next = com.daifuku.mcm.common.ExclusiveLockKey.nextVersion(expected);
        int count = jdbc.update("UPDATE MCM.MCM_MA_KIKIKOSEI SET LASTUPDATE_DT=? WHERE PLANT_ID=? AND KIKIKOSEI_ID=?"
                + " AND ((? IS NULL AND LASTUPDATE_DT IS NULL) OR LASTUPDATE_DT=?)",
                next, plantId, id, expected, expected);
        if (count != 1) throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
        return next;
    }
    ...
    private void saveKoseiRow(KoseiRowForm row, Long plantId,
        ...
        if (isNew) {
            ...
            if (e == null) throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL); // 削除済み(既存チェック)
            // #296追加: 存在はしていても、表示時点から更新日時が変わっていれば他ユーザーの変更とみなす
            if (!isSameLastupdateDt(e.getLastupdateDt(), row.getLastupdateDt())) {
                throw new IllegalStateException(MSG_EXCLUSIVE_CONTROL);
            }
        }

        if (!isNew) now = claimKoseiVersion(plantId, longToBd(row.getKikikoseiId()), row.getLastupdateDt());
    ...
    private void saveKotaiRow(KotaikanriRowForm row, String loginUser,
        ...
        if (!isNew) now = claimVersion("MCM_MA_KIKIKOTAIKANRI", "KOTAIKANRI_ID", longToBd(row.getKotaikanriId()), row.getLastupdateDt());
