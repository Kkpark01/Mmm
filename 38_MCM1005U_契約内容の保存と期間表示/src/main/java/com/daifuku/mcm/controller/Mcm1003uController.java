...
public class Mcm1003uController {
    ...
    public String linkIraiNo(@RequestParam BigDecimal tmKeiyakujikanId, HttpSession session, RedirectAttributes ra) {
        ...
        ra.addAttribute("returnTo", "/mcm1003u");
    ...
    public String linkMitsumoriIrai(@RequestParam BigDecimal tmKeiyakujikanId,
        ...
        // 【不一致修正】MCM1002UのbackハンドラはplantIdクエリが無い場合セッションの
        //   mcm1002u1Formを復元するようになったため、MCM1003U/MCM1008Uから新規に
        //   Deliveryを渡す際は古いmcm1002u1Formが残っていると誤って復元されてしまう。
        //   ここで確実にクリアする。
    ...
    /**
     * 見積複製リンク。
     * 【変換元】Mcm1003uScreen.vb - UVADataGridView_CellContentClick() MITSUMORI_COPY case
     *   プラント件数が1件の場合のみ、確認ダイアログ（MSG_0144「期間情報を複写しますか？」）
     *   の結果でCopyFlgを決定する。「いいえ」を選んでも遷移自体は中止しない（VB仕様）。
     *   プラント件数が2件以上の場合はダイアログを表示せずMCM1008Uへ遷移する（元コードの
     *   ダイアログ処理はCount=1分岐の内側にしかない）。
     * 【不一致修正】#349 確認ダイアログが未実装だったため、画面側でJS confirm→POSTする
     *   方式（見積破棄等と同じパターン）に変更。GETから POST に変更。
     */
    @PostMapping("/link/mitsumori-copy")
    public String linkMitsumoriCopy(@RequestParam BigDecimal tmKeiyakujikanId,
            @RequestParam(required = false) BigDecimal plantId,
            @RequestParam(required = false, defaultValue = "false") boolean copyFlg,
            HttpSession session, RedirectAttributes ra) {
        service.requireUpdate(loginId(session), "MCM1002U");
        var row = findRow(tmKeiyakujikanId, plantId, null);
        ...
        if (plantIds.size() == 1) {
            session.setAttribute("mcm1002uDelivery", delivery(row, plantIds.get(0), Mcm1002uConstants.FUKUSEI, copyFlg));
            session.removeAttribute("mcm1002u1Form");
            session.removeAttribute("mcm1002u2Form");
            return "redirect:/mcm1002u";
        }
