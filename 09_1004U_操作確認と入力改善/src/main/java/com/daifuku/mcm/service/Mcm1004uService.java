...
public class Mcm1004uService extends BaseService {
    ...
    public List<String> validateForTabAdd(Mcm1004uForm f){var e=validatePeriods(f.getTabs());if(!e.isEmpty())return e;var t=f.getTabs().get(f.getTabs().size()-1);if(t.getSyuryoDt()==null)e.add("タブを追加するには最終期間の終了日が必要です。");else if(t.getSyuryoDt().equals(LocalDate.of(9999,12,31)))e.add("9999年を超える期間は追加できません。");return e;}
    ...
    public void updateAll(Mcm1004uForm draft,String user){
        ...
        if(!hasEditableChanges(draft,current))throw new IllegalStateException("データに変更がありません。");
        cleanSupplierEstimates();
    ...
    /** Compare editable values only: selection, audit data and decimal display scale are not edits. */
    private boolean hasEditableChanges(Mcm1004uForm draft,Mcm1004uForm current){
        if(draft.getTabs().size()!=current.getTabs().size())return true;
        for(var t:draft.getTabs()){
            if(t.getTmKikanId()==0)return true;
            var old=current.getTabs().stream().filter(p->Objects.equals(p.getTmKikanId(),t.getTmKikanId())).findFirst().orElseThrow();
            if(!editable(t,"kaisiDt","syuryoDt","tmMitsumoriNo","kaitoDt","biko","syusseinebikiKin").equals(editable(old,"kaisiDt","syuryoDt","tmMitsumoriNo","kaitoDt","biko","syusseinebikiKin")))return true;
            for(var r:t.getTankaRows()){
                var previous=old.getTankaRows().stream().filter(p->same(p.getTmKikimeisaiId(),r.getTmKikimeisaiId())).findFirst().orElseThrow();
                String[] fields={"hyojunKin","sikiriKin","packFlg","keiyakunaiyo","torihosyujikanId","tenkenumu","hosyuhoho","servicekeitai","biko"};
                if(!editable(r,fields).equals(editable(previous,fields)))return true;
            }
            for(var r:t.getTenkenRows()){
                var previous=old.getTenkenRows().stream().filter(p->tenkenKey(p).equals(tenkenKey(r))).findFirst().orElseThrow();
                String[] fields={"tenkenkaisu","tenkenkanoyobi","yakantaioumu","biko"};
                if(!editable(r,fields).equals(editable(previous,fields)))return true;
            }
        }
        return false;
    }
    private List<String> editable(Object row,String...fields){
        var bean=new BeanWrapperImpl(row);var values=new ArrayList<String>();
        for(String field:fields){Object v=bean.getPropertyValue(field);
            if(Set.of("packFlg","tenkenumu","yakantaioumu").contains(field))values.add(flag((String)v));
            else if(bean.getPropertyType(field)==BigDecimal.class&&!field.endsWith("Id"))values.add(str(zero((BigDecimal)v)));
            else values.add(str(v));
        }
        return values;
    }
    /** VB MCM_CLEAN_PAC.SP_TM: same nine predicates/order, in the save transaction.
     * SERIALIZABLE prevents dirty reads of another writer's uncommitted parent rows.
     * Validation/no-change failures never delete data; later save failures roll this back too.
     */
    private void cleanSupplierEstimates(){
        String[][] relations={
            {"MCM_TM_MITSUMORI","MCM_TM_KIKIKOSEI","TM_IRAI_ID","TM_IRAI_ID"},
            {"MCM_TM_MITSUMORI","MCM_TM_KEIYAKUJIKAN","TM_IRAI_ID","TM_IRAI_ID"},
            {"MCM_TM_KEIYAKUJIKAN","MCM_TM_MITSUMORI","TM_IRAI_ID","TM_IRAI_ID"},
            {"MCM_TM_KIKIKOSEI","MCM_TM_MITSUMORI","TM_IRAI_ID","TM_IRAI_ID"},
            {"MCM_TM_KIKIMEISAI","MCM_TM_KIKIKOSEI","TM_KIKIKOSEI_ID","TM_KIKIKOSEI_ID"},
            {"MCM_TM_KOTAIMEISAI","MCM_TM_KIKIMEISAI","TM_KIKIMEISAI_ID","TM_KIKIMEISAI_ID"},
            {"MCM_TM_TANKA","MCM_TM_KIKIMEISAI","TM_KIKIMEISAI_ID","TM_KIKIMEISAI_ID"},
            {"MCM_TM_KIKAN","MCM_TM_KEIYAKUJIKAN","TM_KEIYAKUJIKAN_ID","TM_KEIYAKUJIKAN_ID"},
            {"MCM_TM_TENKEN","MCM_TM_KIKIKOSEI","TM_KIKIKOSEI_ID","TM_KIKIKOSEI_ID"}
        };
        for(var r:relations)jdbc.update("DELETE FROM MCM."+r[0]+" WHERE NOT EXISTS (SELECT 1 FROM MCM."+r[1]+" P WHERE P."+r[3]+"=MCM."+r[0]+"."+r[2]+")");
    }
