...
public class Mcm1002uController {
    ...
    public String index(
            ...
            @RequestParam(defaultValue = "1") int mitsumoriFlg,
            Model model, HttpSession session) {

        Mcm1002u1Form form = new Mcm1002u1Form();
        form.setMitsumoriFlg(mitsumoriFlg);

        // セッションからDelivery情報を取得（MCM1001Uから遷移時）
        Mcm1002uDeliveryDto delivery = (Mcm1002uDeliveryDto) session.getAttribute("mcm1002uDelivery");
        if (delivery != null) {
            plantId = delivery.getPlantId() != null ? delivery.getPlantId().longValue() : plantId;
            tmKeiyakujikanId = delivery.getTmKeiyakujikanId();
        ...
        if (plantId != null) {
            Long keiyakuId = tmKeiyakujikanId != null ? tmKeiyakujikanId : 0L;
            form.setKikikoseiRows(service.loadKikikoseiWithCheckFlags(plantId, keiyakuId));
            form.setKikimeisaiRows(service.loadKikimeisai());
            form.setKotaimeisaiRows(service.loadKotaimeisai(plantId, keiyakuId));
        }
