    ...
    public List<Map<String, Object>> getTorihikisakiCombo() {
        try {
            return jdbc.queryForList(
                ...
                "CASE WHEN YUKO_FLG=0 THEN '' ELSE N'× ' END + TORIHIKISAKI_NK AS TORIHIKISAKI_NK " +
                "FROM MCM.MCM_MA_TORIHIKISAKI " +
                "ORDER BY CASE WHEN YUKO_FLG=0 THEN 0 ELSE 1 END, HYOJIJUN");
        } catch (Exception e) { return List.of(); }
    }
