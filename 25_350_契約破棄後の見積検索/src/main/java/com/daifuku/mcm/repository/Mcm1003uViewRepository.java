...
public class Mcm1003uViewRepository {
    ...
    private List<Mcm1003uRowDto> query(Mcm1003uForm form, BigDecimal id) {
        ...
        // #350: 旧ビューは破棄した契約の状態を優先する。見積へ戻した行は基表で判定する。
        String returnedEstimate = "(TK350.JOTAI='3' AND TM350.JOTAI='1')";
        String effectiveState = "CASE WHEN " + returnedEstimate + " THEN N'見積' ELSE TVA.JOTAI END";
        sql.append("SELECT ");
        sql.append("  TVA.TM_IRAI_ID, ");
        sql.append("  TVA.TM_KEIYAKUJIKAN_ID, ");
        sql.append("  ").append(effectiveState).append(" AS JOTAI, ");
        ...
        sql.append("  CASE WHEN ").append(returnedEstimate).append(" THEN N'破棄' ELSE TVA.MITSUMORI_DEL END AS MITSUMORI_DEL, ");
        sql.append("  CASE WHEN ").append(returnedEstimate).append(" THEN N'契約' ELSE TVA.KEIYAKU_KEIYAKU END AS KEIYAKU_KEIYAKU, ");
        sql.append("  CASE WHEN ").append(returnedEstimate).append(" THEN NULL ELSE TVA.KEIYAKU_DEL END AS KEIYAKU_DEL, ");
        sql.append("  TVA.TK_KEIYAKU_ID, ");
        sql.append("  TVA.SHONINJOTAI, ");
        sql.append("  TVA.KAITOKIZITSU_DT ");
        sql.append("FROM MCM.MCM_1003_V TVA ");
        sql.append("LEFT JOIN MCM.MCM_TM_KEIYAKUJIKAN TM350 ON TM350.TM_KEIYAKUJIKAN_ID=TVA.TM_KEIYAKUJIKAN_ID ");
        sql.append("LEFT JOIN MCM.MCM_TK_KEIYAKU TK350 ON TK350.TK_KEIYAKU_ID=TVA.TK_KEIYAKU_ID ");
        ...
        if (isNotEmpty(jotaiList)) {
            sql.append("AND CHARINDEX(").append(effectiveState).append(", :jotaiList) > 0 ");
