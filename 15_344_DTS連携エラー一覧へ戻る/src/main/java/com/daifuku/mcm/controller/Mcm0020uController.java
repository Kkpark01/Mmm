...
//	    西暦年を表す "uuuu"（Year）を使用する
    ...
    private static final String RETURN_STATES = "mcm0020u_returnStates";
    private record ReturnState(Mcm0020uForm form, List<DtslogSearchResultDto> results, boolean linkEnabled) {}

    private static Mcm0020uForm copyForm(Mcm0020uForm original) {
        var copy = new Mcm0020uForm();
        copy.setSearchMode(original.getSearchMode());
        copy.setLogDateFrom(original.getLogDateFrom());
        copy.setLogDateTo(original.getLogDateTo());
        return copy;
    }

    @SuppressWarnings("unchecked")
    private void remember(Model model, HttpSession session, Mcm0020uForm form,
                          List<DtslogSearchResultDto> results, boolean linkEnabled) {
        String token = java.util.UUID.randomUUID().toString();
        synchronized (session) {
            var states = (java.util.LinkedHashMap<String, ReturnState>) session.getAttribute(RETURN_STATES);
            if (states == null) states = new java.util.LinkedHashMap<>();
            // タブごとの一覧を保持しつつ、セッション内の無制限な蓄積を防ぐ。
            while (states.size() >= 5) states.remove(states.keySet().iterator().next());
            states.put(token, new ReturnState(copyForm(form), List.copyOf(results), linkEnabled));
            session.setAttribute(RETURN_STATES, states);
        }
        model.addAttribute("returnStateToken", token);
    }

    @GetMapping("/return")
    @SuppressWarnings("unchecked")
    public String returnFromPlant(@org.springframework.web.bind.annotation.RequestParam(required=false) String state,
                                  Model model, HttpSession session) {
        ReturnState saved;
        synchronized (session) {
            var states = (java.util.Map<String, ReturnState>) session.getAttribute(RETURN_STATES);
            saved = states == null ? null : states.get(state);
        }
        if (saved == null) {
            String view = index(model, session);
            model.addAttribute("errors", List.of("検索結果の保持期限が切れました。検索をやり直してください。"));
            return view;
        }
        model.addAttribute("screenTitle", getScreenTitle());
        model.addAttribute("screenId", getFunctionId());
        model.addAttribute("form", copyForm(saved.form()));
        model.addAttribute("results", saved.results());
        model.addAttribute("resultCount", saved.results().size());
        model.addAttribute("showLink", hasUpdateAuthority());
        model.addAttribute("linkEnabled", saved.linkEnabled() && hasUpdateAuthority());
        model.addAttribute("returnStateToken", state);
        model.addAttribute("restoreReturnState", true);
    ...
    public String search(@ModelAttribute Mcm0020uForm form,
        ...
        remember(model, session, form, results, linkEnabled);
    ...
    private List<String> validateSearchForm(Mcm0020uForm form) {
        ...
        if (Mcm0020uConstants.MODE_DTSLOG.equals(form.getSearchMode())) {
            ...
            if (!isValidLogDate(form.getLogDateFrom())) {
                errors.add(Mcm0020uConstants.MSG_LOG_DATE_FROM_INVALID);
            }
            if (!isValidLogDate(form.getLogDateTo())) {
                errors.add(Mcm0020uConstants.MSG_LOG_DATE_TO_INVALID);
            }

            // ▼▼▼ #299対応：出力日FROM > 出力日TOの前後関係チェックを追加 ▼▼▼
            // 書式が両方とも正しい場合のみ前後関係を比較する（不正日付での比較・例外を防止）
            if (errors.isEmpty()
                    && !isBlank(form.getLogDateFrom())
                    && !isBlank(form.getLogDateTo())) {
                LocalDate from = LocalDate.parse(form.getLogDateFrom(), LOG_DATE_FORMATTER);
                LocalDate to = LocalDate.parse(form.getLogDateTo(), LOG_DATE_FORMATTER);
                if (from.isAfter(to)) {
                    errors.add(Mcm0020uConstants.MSG_0027);
                }
            }
            // ▲▲▲ #299対応 ▲▲▲
        }
