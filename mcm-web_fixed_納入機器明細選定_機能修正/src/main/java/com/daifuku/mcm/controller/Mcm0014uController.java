...
public class Mcm0014uController extends BaseController {
    ...
    private List<String> search(Work w) {
        ...
        if(results.isEmpty())return List.of("検索結果が1件も存在しません。");
        return List.of();
    }
    ...
    private List<String> validate(Work w) {
        ...
        for(var row:active) {
            ...
            if(str(row.getBiko()).getBytes(java.nio.charset.Charset.forName("Shift_JIS")).length>4000)return List.of("備考は4000桁以下で入力してください。");
