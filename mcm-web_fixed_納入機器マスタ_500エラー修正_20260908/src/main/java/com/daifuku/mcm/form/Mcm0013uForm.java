...
public class Mcm0013uForm implements Serializable {
    ...
    public Mcm0013uForm copy() {
        // DevToolsではappとRestartClassLoaderが同名クラスを持つ場合がある。
        // 復元時はこのフォームのローダーを明示し、明細・個体・表示値も深く複製する。
        byte[] data = org.springframework.util.SerializationUtils.serialize(this);
        try (var input = new org.springframework.core.ConfigurableObjectInputStream(
                new java.io.ByteArrayInputStream(data), Mcm0013uForm.class.getClassLoader())) {
            return (Mcm0013uForm) input.readObject();
        } catch (java.io.IOException | ClassNotFoundException e) {
            throw new IllegalStateException("納入機器マスタの作業データを複製できませんでした。", e);
        }
    }
