...
public class Mcm2003uService {
    ...
    @Transactional
    public void save(Mcm2003uForm form,String user){
        requireEditable(form);
        if (!hasChanges(form, repo.findKihonMitsumori(form.getUmKihonMitsumoriId())))
            throw new com.daifuku.mcm.exception.DataNotChangedException();
        repo.updateBrandMethods(form,user);repo.updateKihonMitsumori(form,user);
        integrity.estimate(form.getUmKihonMitsumoriId());totals.recalculate(form.getUmKihonMitsumoriId(),user);
    }

    /** VB checkChangedStatus相当。表示専用項目・更新日時は変更判定に含めない。 */
    private boolean hasChanges(Mcm2003uForm form, Mcm2003uForm current) {
        var input = new org.springframework.beans.BeanWrapperImpl(form);
        var original = new org.springframework.beans.BeanWrapperImpl(current);
        for (String key : java.util.List.of("umMitsumoriNo", "iraitenpoId", "sofutenpoId",
                "keiyakujikantai", "mitsumoriLevel", "mitsumorigiken", "iraitantoNk",
                "sofutantoNk", "mitsumoriJouken", "hosyuhoho", "biko"))
            if (!sameInput(input.getPropertyValue(key), original.getPropertyValue(key))) return true;
        for (var brand : form.getBrandRows()) {
            if (brand.isSoftHosyuhohoEdited() && !sameInput(brand.getSoftHosyuhoho(), brand.getOriginalSoftHosyuhoho())) return true;
            for (var entry : brand.getEditedOriginals().entrySet())
                if (!sameInput(com.daifuku.mcm.common.Mcm2003uEdits.get(brand, entry.getKey()), entry.getValue())) return true;
        }
        for (var row : form.getKoseiRows())
            for (var entry : row.getEditedOriginals().entrySet())
                if (!sameInput(com.daifuku.mcm.common.Mcm2003uEdits.get(row, entry.getKey()), entry.getValue())) return true;
        return false;
    }

    private boolean sameInput(Object a, Object b) {
