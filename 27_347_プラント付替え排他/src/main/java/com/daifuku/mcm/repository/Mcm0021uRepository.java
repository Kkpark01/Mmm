...
public class Mcm0021uRepository {
    ...
    /**
     * 納入先を動的条件で検索する
     * @param nonyusakiCd 納入先コード（前方一致、大文字小文字区別あり）
     * @param nonyusakiNk 納入先名（部分一致、旧名・工場名・カナ名・英名を含む5項目をOR横断検索。英名のみ大文字小文字無視）
     * @param supportId サポートID（前方一致、大文字小文字区別あり）
    ...
    public List<Mcm0021uNonyusakiDto> searchNonyusaki(
        ...
        /*
         * 【チケット#327】検索方式修正
         *   4条件はそれぞれ独立した検索方式・対象カラムを持つ（VB版
         *   Mcm0021u_MCM_MA_NONYUSAKITableAdapter.xmlの仕様に準拠）。
         *   - 納入先コード: 前方一致、大文字小文字区別あり（単一カラム）
         *   - 納入先名: 部分一致、大文字小文字無視（5項目のみをOR横断検索）
         *   - サポートID: 前方一致、大文字小文字区別あり（単一カラム）
         *   - プラント名: 部分一致、大文字小文字無視（単一カラム）
         */
        if (!isEmpty(nonyusakiCd)) {
            sql.append("AND MAA.NONYUSAKI_CD LIKE ? ");
        ...
        if (!isEmpty(supportId)) {
            sql.append("AND MAB.SUPPORT_ID LIKE ? ");
    ...
    public List<Mcm0021uKikikoseiDto> searchKikikosei(
         ...
         * 【チケット#327】検索方式修正
         *   4条件はそれぞれ独立した検索方式・対象カラムを持つ（VB版
         *   Mcm0021u_MCM_MA_KIKIKOSEITableAdapter.xmlの仕様に準拠）。
         *   - 納入先コード: 前方一致、大文字小文字区別あり（単一カラム）
         *   - 納入先名: 部分一致、大文字小文字無視（5項目のみをOR横断検索）
         *   - サポートID: 前方一致、大文字小文字区別あり（単一カラム）
         *   - プラント名: 部分一致、大文字小文字無視（単一カラム）
        ...
        if (!isEmpty(nonyusakiCd)) {
            ...
            sql.append("  WHERE MAB.PLANT_ID = MAE.PLANT_ID AND MAA.NONYUSAKI_CD LIKE ?) ");
            params.add(nonyusakiCd + "%");
        }
        if (!isEmpty(nonyusakiNk)) {
            sql.append("AND EXISTS (SELECT 1 FROM MCM.MCM_MA_NONYUSAKI MAA ");
        ...
        if (!isEmpty(supportId)) {
            sql.append("AND EXISTS (SELECT 1 FROM MCM.MCM_MA_NONYUSAKI MAA ");
            sql.append("  INNER JOIN MCM.MCM_MA_PLANT MAB ON MAB.NONYUSAKI_ID = MAA.NONYUSAKI_ID ");
            sql.append("  WHERE MAB.PLANT_ID = MAE.PLANT_ID AND MAB.SUPPORT_ID LIKE ?) ");
            params.add(supportId + "%");
        }
        if (!isEmpty(plantNk)) {
            sql.append("AND EXISTS (SELECT 1 FROM MCM.MCM_MA_NONYUSAKI MAA ");
    ...
    /** #347: 更新日時を含む元プラントの構成・明細・個体を比較する。
     * SERIALIZABLEの呼出元で読み、比較後の更新までロックを保持する。 */
    public String sourceFingerprint(BigDecimal plantId) {
        try {
            var digest = java.security.MessageDigest.getInstance("SHA-256");
            String[] queries = {
                "SELECT k.* FROM MCM.MCM_MA_KIKIKOSEI k WHERE k.PLANT_ID = ? ORDER BY k.KIKIKOSEI_ID",
                "SELECT d.* FROM MCM.MCM_MA_KIKIMEISAI d WHERE EXISTS (SELECT 1 FROM MCM.MCM_MA_KIKIKOSEI k WHERE k.KIKIKOSEI_ID=d.KIKIKOSEI_ID AND k.PLANT_ID=?) ORDER BY d.KIKIMEISAI_ID",
                "SELECT i.* FROM MCM.MCM_MA_KIKIKOTAIKANRI i WHERE EXISTS (SELECT 1 FROM MCM.MCM_MA_KIKIKOSEI k WHERE k.KIKIKOSEI_ID=i.KIKIKOSEI_ID AND k.PLANT_ID=?) ORDER BY i.KOTAIKANRI_ID"
            };
