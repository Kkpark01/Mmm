...
public class Mcm0021uService {
    ...
    /**
     * 【移植】担当者の実効権限区分（"UPDATE"/"INSPECTION"/"NONE"）を取得する。
     * 元VB: CPCoreUserControl.GetAuthorityDivision() → CPUserInfo.GetAuthorityDivision(functionId)
     *   「納入機器関連」（KENGENBUNRUI_ID=1）はMCM0021U（プラント付替え）のKINO_IDにも
     *   紐づくため、機能IDにMCM0021Uを渡せば当該カテゴリの実効権限が得られる。
     *
     * @param loginId    ログインID
     * @param functionId 機能ID（画面ID）
     * @return AppConstants.AUTHORITY_DIVISION_UPDATE / INSPECTION / NONE
     */
    ...
    /** #298: 付替え対象個体の有無に依存せず、遷移元が指定したプラントを表示する。 */
    public Mcm0021uNonyusakiDto findPlantInfoById(BigDecimal plantId) {
        return mcm0021uRepository.findPlantInfoById(plantId);
    }
     ...
     */
    public static final String CONFLICT = "他のユーザがデータを変更した可能性があります。処理をやり直してください。";
    public record SourceState(BigDecimal plantId, String fingerprint, List<Mcm0021uMotoRowDto> rows) {}

    @Transactional(isolation = org.springframework.transaction.annotation.Isolation.SERIALIZABLE)
    public SourceState loadSource(BigDecimal plantId) {
        String fingerprint = mcm0021uRepository.sourceFingerprint(plantId);
        return new SourceState(plantId, fingerprint, findMotoByPlantId(plantId));
    }
    ...
    public String executeReplace(BigDecimal plantIdMoto, BigDecimal plantIdSaki,
                                  List<MotoRowForm> motoRows, String loginUser, SourceState snapshot) {
        var errors = validateReplace(plantIdMoto, plantIdSaki, motoRows);
        if (!errors.isEmpty()) return errors.get(0);

        // #347: 古い表示内容からの保存は、書込みを始める前に拒否する。
        if (snapshot == null || snapshot.plantId() == null || snapshot.plantId().compareTo(plantIdMoto) != 0
                || snapshot.fingerprint() == null
                || !snapshot.fingerprint().equals(mcm0021uRepository.sourceFingerprint(plantIdMoto)))
            return CONFLICT;
        ...
        for (var group : groups.entrySet()) {
            ...
            if (actualRows != rows.size()) return CONFLICT;
