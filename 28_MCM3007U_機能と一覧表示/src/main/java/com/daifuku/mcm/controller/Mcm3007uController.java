...
@Controller @RequestMapping("/mcm3007u") public class Mcm3007uController extends BaseController {
 ...
 @GetMapping("/link/{destination}") public String link(@PathVariable String destination,@RequestParam String kind,@RequestParam int rowIndex,HttpServletRequest request,HttpSession s,RedirectAttributes ra){synchronized(s){try{
  ...
  if(destination.equals("mcm0013u")&&!service.hasBrand(id(d,"PLANT_ID")))throw new IllegalStateException("ブランド構成が登録されていません。");
  ...
  switch(destination){
   ...
   case "mcm1004u": if(!kind.equals("tk")||id(d,"TM_KEIYAKUJIKAN_ID")==null)throw new IllegalStateException(EXPIRED);u.queryParam("tmKeiyakujikanId",d.get("TM_KEIYAKUJIKAN_ID")).queryParam("seniMotoKbn",1).queryParam("returnTo","/mcm3007u");break;
