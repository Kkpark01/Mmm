...
@Repository public class Mcm3007uRepository {
 ...
 public List<Grid3RowForm> searchTka(String cd,String nk,String sid,String pnk,List<String> status){
  if(status.isEmpty())return List.of();var st=states(status);var args=new ArrayList<Object>(st);
  var sql=new StringBuilder("SELECT DISTINCT V.*,B.UM_KIHON_MITSUMORI_ID,K.LASTUPDATE_DT AS CONTRACT_VERSION,CASE WHEN K.JOTAI='3' AND TM.JOTAI='1' THEN N'見積' ELSE V.JOTAI END AS EFFECTIVE_JOTAI,CASE WHEN K.JOTAI='3' AND TM.JOTAI='1' THEN 1 ELSE 0 END AS RETURNED_ESTIMATE,TRY_CAST(V.KEIYAKUJIKANTAI AS DECIMAL) AS SORT_HOURS FROM MCM.MCM_1003_V V JOIN MCM.MCM_TM_KIKIKOSEI C ON C.TM_IRAI_ID=V.TM_IRAI_ID JOIN MCM.MCM_TM_KIKIMEISAI D ON D.TM_KIKIKOSEI_ID=C.TM_KIKIKOSEI_ID JOIN MCM.MCM_TM_KOTAIMEISAI E ON E.TM_KIKIMEISAI_ID=D.TM_KIKIMEISAI_ID JOIN MCM.MCM_UM_KOTAIMEISAI UE ON UE.KOTAIKANRI_ID=E.KOTAIKANRI_ID JOIN MCM.MCM_UM_KIKIMEISAI UD ON UD.UM_KIKIMEISAI_ID=UE.UM_KIKIMEISAI_ID JOIN MCM.MCM_UM_KIKIKOSEI UC ON UC.UM_KIKIKOSEI_ID=UD.UM_KIKIKOSEI_ID JOIN MCM.MCM_UM_KIHON_BRAND B ON B.UM_KIHON_BRAND_ID=UC.UM_KIHON_BRAND_ID LEFT JOIN MCM.MCM_TK_KEIYAKU K ON K.TK_KEIYAKU_ID=V.TK_KEIYAKU_ID LEFT JOIN MCM.MCM_TM_KEIYAKUJIKAN TM ON TM.TM_KEIYAKUJIKAN_ID=V.TM_KEIYAKUJIKAN_ID WHERE (CASE WHEN K.JOTAI='3' AND TM.JOTAI='1' THEN N'見積' ELSE V.JOTAI END) IN ("+marks(st.size())+")");filters(sql,args,"V",cd,nk,sid,pnk,true);sql.append(" ORDER BY V.TM_IRAI_NO DESC,SORT_HOURS,V.TM_KEIYAKUJIKAN_ID,B.UM_KIHON_MITSUMORI_ID");
  return jdbc.queryForList(sql.toString(),args.toArray()).stream().map(m->{var d=display(m);d.put("JOTAI",state(d.get("JOTAI")));d.put("JOTAI",state(d.remove("EFFECTIVE_JOTAI")));if("1".equals(d.remove("RETURNED_ESTIMATE"))){d.put("KEIYAKU_KEIYAKU","契約");d.put("KEIYAKU_DEL","");}d.remove("SORT_HOURS");
   if(!d.getOrDefault("FUSEI","").isBlank())d.put("KEIYAKU_NO",d.getOrDefault("KEIYAKU_NO","")+" "+d.get("FUSEI"));var r=new Grid3RowForm();r.setDisplay(d);r.setTkKeiyakuId(id(d,"TK_KEIYAKU_ID"));r.setTmKeiyakujikanId(id(d,"TM_KEIYAKUJIKAN_ID"));r.setJotai(d.get("JOTAI"));r.setIraino(d.get("TM_IRAI_NO"));r.setTorihikisakiCd(d.get("TORIHIKISAKI_CD"));r.setTorihikisakiNk(d.get("TORIHIKISAKI_NK"));r.setNonyusakiNk(d.get("NONYUSAKI_NK"));r.setSupportId(d.get("SUPPORT_ID"));r.setPlantNk(d.get("PLANT_NK"));r.setKeiyakuNo(d.get("KEIYAKU_NO"));r.setKeiyakuDt(d.get("KEIYAKU_DT"));r.setKaiyakuDt(d.get("KAIYAKU_DT"));r.setShoninJotai(d.get("SHONINJOTAI"));return r;}).toList();
 }
